...
当前状态：**Day 7 / 7 已完成**。传统视觉扫描、失败诊断、Streamlit 界面和自动回归测试均已接通。
...
| Day 5 | 完整传统视觉流水线、扫描结果增强、失败检测 | 已完成 |
| Day 6 | Streamlit 界面、过程展示和文件下载 | 已完成 |
| Day 7 | 多场景测试、问题修复、演示与文档整理 | 已完成 |
...
| 文件 | 内容 |
|---|---|
| [`work/day05_robust_scanner.py`](work/day05_robust_scanner.py) | 统一扫描入口、输入验证、失败分类、诊断信息和输出质量检查 |
| [`work/day05_failure_cases.py`](work/day05_failure_cases.py) | 空白、小轮廓、圆形及异常二值结果等可控失败测试 |
| [`work/day05_realistic_cases.py`](work/day05_realistic_cases.py) | 低对比度、严重模糊、画面裁切和 Canny 参数实验 |

## Day 6 应用

| 文件 | 内容 |
|---|---|
| [`work/day06_app.py`](work/day06_app.py) | Streamlit 上传、诊断与过程展示、参数滑块、质量指标和三类结果下载 |

## Day 7 测试与交付

| 文件 | 内容 |
|---|---|
| [`work/day07_regression.py`](work/day07_regression.py) | 8个成功/失败场景、输出契约、运行时间和角点排序边界测试 |

最终回归集包含基础文档、彩色文档、低对比度文档、空白图、圆形图、小型矩形、严重模糊和角点裁切，当前预期结果全部通过。对称菱形测试曾暴露坐标和/差排序的并列值缺陷，现已改为绕中心按角度排序，并通过全部旧样例回归。
...
- 统一扫描入口使用固定结构同时返回成功状态、失败原因、诊断数据、中间图和彩色/灰度/黑白结果；预期失败不会继续进入无效的透视和增强步骤。
- Streamlit 页面能够解码内存上传文件、缓存不同图像与参数组合的扫描结果，并对正常文档和不可识别输入提供不同反馈。
...
```powershell
python -m streamlit run work/day06_app.py
```

运行最终回归测试：

```powershell
python work/day07_regression.py
```
...
- 最适合单张矩形文档：四角完整可见、外边界连续、文档与背景存在明显亮度梯度，且文档约占画面的 10%～95%。
- 允许透视和一定角度的旋转；背景可以包含小型杂物，但明显大于文档的矩形边框可能被优先误检。
- 严重模糊、边界遮挡或角点出画会破坏完整轮廓。闭运算只能连接有限尺度的断口，不能恢复不存在的真实边缘。
- 当前输出固定采用 A4 的 `210:297` 宽高比。收据、名片等非 A4 文档即使检测成功，也可能被纵向拉伸或压缩。
- Canny 和最小面积阈值用于控制候选范围；持续降低门槛会引入弱边缘、噪声和小型误检，不能保证恢复正确文档。
- 当前没有手动角点修正、旋转方向选择和多文档检测；自动结果不正确时应明确失败或更换输入照片，而不应将输出视为可靠扫描件。
...
`inputs/document_color.png` 是 2026-09-07 使用 OpenAI 内置图像生成工具制作的彩色合成测试图，包含醒目的红、蓝、绿、橙色区域，用于验证网页预览、BGR/RGB 转换、透视校正和下载文件的颜色是否一致。
